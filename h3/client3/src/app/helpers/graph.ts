...
export function colorScale(value: number, min: number = -10.0, max: number = 50.0): { r: any; g: any; b: any; } {  
  ...
  function scaleInterpolation(scale: Scale, value: number, smooth = true): [number, number, number] {
    const keys = Object.keys(scale).map(k => +k).sort();  // js-objects don't keep the ordering of numeric keys as they were entered. instead it goes: integers, strings, decimals, each group sorted by first appearance, not value.
